...
import pickle           #Converting to byte stream or inverse
...
'''
def read_lines(filename):                                               #reads lines and splits based on newline character
...
'''
def split_line(line):
    return line.split('.')              #Splitting based on  '.' / Have already black listed other punctuations
...
def process_data():
   ...
   # print('\nq : {0} ; a : {1}'.format(qlines[60]))
    #print('\nq : {0} ; a : {1}'.format(qlines[61]))
    ...
    #print('\nq : {0} ; a : {1}'.format(qtokenized[60]))
    #print('\nq : {0} ; a : {1}'.format(qtokenized[61]))


    # indexing -> idx2w, w2idx : en/ta
    print('\n >> Index words')
    idx2w, w2idx, freq_dist = index_( qtokenized, vocab_size=VOCAB_SIZE)
